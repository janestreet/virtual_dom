...
module%test [@name "Effect.Result"] _ = struct
  ...
  let test_effect effect_ =
    Effect.Expert.handle
      ~on_exn:raise
      (let%bind.Effect effect_ in
       Effect.print_s [%message (effect_ : (string, string) Result.t)])
  ;;
  ...
  let%expect_test "monad bind works as expected" =
    ...
    in
    test_effect effect_;
    [%expect {| (effect_ (Error "I am printing but I'm the failure type")) |}]
  ;;
...
module%test [@name "Combining Effect.Result"] _ = struct
  ...
  let test_effect effect_ =
    Effect.Expert.handle
      ~on_exn:raise
      (let%bind.Effect effect_ in
       Effect.print_s [%message (effect_ : (C.t, Error.t) Result.t)])
  ;;
  ...
  let%expect_test "Fail both" =
    ...
    Effect.Result.combine a b ~ok:C.from_a_b ~err:combine_errors |> test_effect;
    [%expect
      {| (effect_ (Error ("combining two errors" (err_a "fail a") (err_b "fail b")))) |}]
  ;;
end
...
module%test [@name "Effect.Or_error"] _ = struct
  ...
  let test_effect effect_ =
    Effect.Expert.handle
      ~on_exn:raise
      (let%bind.Effect effect_ in
       Effect.print_s [%message (effect_ : string Or_error.t)])
  ;;
