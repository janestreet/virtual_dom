...
let terminal =
  ...
  fun () ->
    ...
    let interpret command (term : terminal Js.t) =
      ...
      | _ ->
        term##echo (Js.string "Invalid command")
    in
    let completion term _command k =
      ...
      let completions =
        ...
        | ["get"; key] ->
          List.filter (Hashtbl.keys values)
            ~f:(String.is_prefix ~prefix:key)
        | _ -> []
      in
      let completions = Js.array (Array.of_list_rev_map ~f:Js.string completions) in
      let open Js.Unsafe in
      fun_call k [| inject completions |]
    in
    let options =
      object%js
...
let view =
  fun count ->
    Node.div []
      [ Node.div [] [Node.text (Int.to_string count)]
      ; terminal ()
      ]
;;
