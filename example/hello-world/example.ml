...
let view =
  let chars = [|'h'; 'e'; 'l'; 'l'; 'o'; '_'; 'w'; 'o'; 'r'; 'l'; 'd'; '_'|] in
          ...
          Tyxml.Svg.
            [ rect
                ~a:
                  [ a_width (300., None)
                  ; a_height (100., None)
                  ; a_style (Printf.sprintf "fill:rgb(%d,%d,%d);" (11 * k mod 255) 0 0)
                  ]
                []
            ; circle
                ~a:
                  [ a_cx ((5. *. cos (float_of_int k)) +. 40., None)
    ...
    Dom.handler (fun _ ->
      ...
      Dom_html.window##setInterval
        (Js.wrap_callback (fun _ ->
           ...
           elt :=
             Node.Patch.apply
               (Node.Patch.create ~previous:!vdom ~current:new_vdom)
               !elt;
           vdom := new_vdom))
        100.
      |> ignore;
