(** A css3 tokenizer.
    ...
    - This does not implement BAD_STRING, BAD_URI, BAD_COMMENT That is we are not
      implementing any support for robustness in the face of bad input. Rationale is we
      rather learn that we wrote invalid CSS.
    - This does not support escape sequence outside of strings.
    - We didn't do anything to validate the unicode support.

    Also we generally only implement what we need to parse declaration lists. Not
    everything needed to parse complete css style sheets (e.g. cdo, cdc). *)
open! Core
