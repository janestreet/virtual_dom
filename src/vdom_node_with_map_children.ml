...
module Input = struct
  type ('k, 'cmp) t =
    { children : ('k, Node.t, 'cmp) Map.t
    ; tag : string
    ; attr : Attr.t option
    ; is_svg : bool
    }

  let to_empty_vdom { tag; attr; is_svg; children = _ } =
    let creator = if is_svg then Node.create_svg else Node.create in
    creator tag ?attrs:(Option.map attr ~f:(fun attr -> [ attr ])) []
  ;;
end

let diff_patch ~prev ~next ~element =
  let diff = Node.Patch.create ~previous:prev ~current:next in
  let (_ : Js_of_ocaml.Dom_html.element Js.t) = Node.Patch.apply diff element in
  ()
;;
...
module Widget (K : Comparator.S) = struct
  ...
  module State = struct
    type t =
      { elements : (Node.t * Dom_html.element Js.t) Map.M(K).t
      ; me : dom Js.t
      }
  ...
  let do_change ~key_before ~key_after ~elements current_vdom =
    ...
    let patch = Node.Patch.create ~previous:old_vdom ~current:current_vdom in
    let current_element =
      (* if there isn't a patch, we can use the old element without issue *)
      if Node.Patch.is_empty patch
      then old_element
      else Node.Patch.apply patch old_element
    in
    let elements =
      let elements =
  ...
  let create input =
    let dom = Node.to_dom (Input.to_empty_vdom input) in
    let elements =
      Map.map input.Input.children ~f:(fun vdom ->
        let element = Node.to_dom vdom in
  ...
  let to_vdom_for_testing =
    `Custom
      (fun { Input.children; attr; is_svg = _; tag } ->
        Node.create
          tag
...
module Instancer : sig
  val get : map:('k, Node.t, 'cmp) Map.t -> ('k, 'cmp) Input.t -> Node.t
end = struct
  ...
  let get (type k cmp) ~(map : (k, Node.t, cmp) Map.t) : (k, cmp) Input.t -> Node.t =
    ...
    | None ->
      ...
      in
      let f = unstage (Node.widget_of_module (module M)) in
      Weak_map.set instances key ((Obj.repr : (_ Input.t -> Node.t) -> Obj.t) f);
      f
  ;;
end

let make ?(is_svg = false) ~tag ?attr children =
  let f = Instancer.get ~map:children in
  f { Input.children; tag; is_svg; attr }
;;
...
let ol ?attrs m = create "ol" ?attrs m
