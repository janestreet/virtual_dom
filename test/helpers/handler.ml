...
let throwing_proxy =
  ...
  in
  let create_proxy
    : Js.Unsafe.any -> Js.Unsafe.any -> Js.Unsafe.any -> Js.Unsafe.any -> Js.Unsafe.any
    =
    (* Given a base-object, a function that raises an ocaml exception, a function that
       unwraps a ref, and an optional "path" to the base-object, return a proxy object
       that behaves exactly like a read-only [base], but any accesses into that object
       which aren't present instead call [raise_missing_field_path] instead of returning
       [undefined]. The objects returned via successful indexes into [base] are also
       wrapped in a proxy of the same design.
    ...
    Js.Unsafe.pure_js_expr
      {js|
        (function create_proxy (base, raise_missing_field_path, bang, path) {
          path = path || [];
          function get(obj, field) {
            if (field in obj) {
              var field_value = obj[field];
              if (field === 'defaultPrevented') {
                // Special casing to get value from ref
                return (bang (field_value));
              } else if (typeof field_value === 'object') {
                return create_proxy(obj[field], raise_missing_field_path, bang, path.concat([field]));
              } else {
                return field_value;
              }
            } else {
  ...
  fun base ->
    ...
    let bang = Ref.( ! ) |> Js.wrap_callback |> Js.Unsafe.inject in
    let path = Js.Unsafe.inject Js.null in
    Js.Unsafe.fun_call create_proxy [| base; throwing_function; bang; path |]
;;
